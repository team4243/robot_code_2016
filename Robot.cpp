	...
	Robot() :
	 ...
	 SmartDashboard::PutBoolean("Disabled",m_disabled);
		...
		void TeleopPeriodic() {
			...
			SmartDashboard::PutNumber("lift speed setting ", liftSpeed) ;
...
//			// Do NOT trigger lift during the no-climb time.
				...
				if (m_benController.GetRawButton(YellowBtn)) {
					...
					if (safetyCounter >= safetyCounts) { // Past the safety-wait time.
						...
						if (m_lizardLimitSwitch.Get()) {
							...
							if (lizardFlag) {
								m_lizardLiftMotor.SetSpeed(liftSpeed);//Run lizard lift motor at a quarter power
							}//endif lizardFlag
			...
			if (m_benController.GetRawButton(GreenBtn)  ) {
				...
				m_lizardLiftMotor.SetSpeed(-0.1 );
				safetyCounter = 0;
			...
			if (m_benController.GetPOV(POVAngle) == 0) {
				...
				m_winchMotor.SetSpeed(0.25);//Run winch motor at a quarter power
			}
